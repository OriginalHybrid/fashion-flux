...
class LeffaTransform(nn.Module):
    ...
    def forward(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        src_image_list, ref_image_list, mask_list, densepose_list = [], [], [], []
        for i in range(batch_size):
            ...
            src_image = self.vae_processor.preprocess(src_image, self.height, self.width)[0]
            ref_image = self.vae_processor.preprocess(ref_image, self.height, self.width)[0]
            mask = self.mask_processor.preprocess(mask, self.height, self.width)[0]
            if self.dataset == "pose_transfer":
                densepose = densepose.resize((self.width, self.height), Image.NEAREST)
            else:
                densepose = self.vae_processor.preprocess(densepose, self.height, self.width)[0]

            src_image = self.prepare_image(src_image)
            ref_image = self.prepare_image(ref_image)
            mask = self.prepare_mask(mask)
            densepose = self.prepare_densepose(densepose) if self.dataset == "pose_transfer" else self.prepare_image(densepose)
        ...
        batch["src_image"] = torch.cat(src_image_list, dim=0)
        batch["ref_image"] = torch.cat(ref_image_list, dim=0)
        batch["mask"] = torch.cat(mask_list, dim=0)
        batch["densepose"] = torch.cat(densepose_list, dim=0)
        return batch
